...
6. 수식: 소스 하네스들은 표준 LaTeX `$...$`/`$$...$$`로 통일돼 있다 (learning format_spec §13 · essay article_spec §4 · fiction episode_spec §3) — **수식은 무변환**(KaTeX가 그대로 렌더). 정리/증명은 `> 📐 **정리|보조정리|정의|증명 N.M (이름)**: …` blockquote 마커로 들어온다 — **연속된 `>` 줄 한 덩어리**를 유형에 따라 `:::theorem`/`:::lemma`/`:::definition`/`:::proof` 박스로 기계 변환하고, 📐와 증명 끝 ∎는 제거한다(∎는 블로그 CSS 자동, HANDBOOK §4.4). 이름이 있으면 `{title="N.M 이름"}`으로 옮긴다. 소스에 `:::`가 직접 있으면 규약 위반 — 변환은 해주되 원 하네스에 보고.
