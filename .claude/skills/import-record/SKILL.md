...
6. 수식: 소스 하네스들은 표준 LaTeX `$...$`/`$$...$$`로 통일돼 있다 (study-loop은 게이트 R-CH-011, blog-loop은 CLAUDE.md §표기 규약 — 소재는 `docs/harness-guide.md` §4.5 표) — **수식은 무변환**(KaTeX가 그대로 렌더). 캐럿 지수(`2^10`)가 들어오면 규약 위반이니 고치지 말고 원 하네스에 보고한다 — 여기서 고치면 정본이 갈라진다. 정리/증명은 `> 📐 **정리|보조정리|정의|증명 N.M (이름)**: …` blockquote 마커로 들어온다 — **연속된 `>` 줄 한 덩어리**를 유형에 따라 `:::theorem`/`:::lemma`/`:::definition`/`:::proof` 박스로 기계 변환하고, 📐와 증명 끝 ∎는 제거한다(∎는 블로그 CSS 자동, HANDBOOK §4.4). 이름이 있으면 `{title="N.M 이름"}`으로 옮긴다. 소스에 `:::`가 직접 있으면 규약 위반 — 변환은 해주되 원 하네스에 보고.
