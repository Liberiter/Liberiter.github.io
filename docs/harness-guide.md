...
├─ .claude/
│  ├─ skills/                      ← ② 스킬 3종 — /이름 으로 호출하는 작업 절차서
...
2. **변환은 블로그가 소유하고, 저작은 밖에서 한다.** 원고를 만드는 하네스(`blog-loop`·
   `study-loop`)는 블로그 스키마를 전혀 모른다. 스키마·문법·세계관 규칙은 전부 이 저장소의
   지식이므로 "원고 → 기록" 변환도 이 저장소의 스킬이 담당한다. 블로그가 바뀌어도 고칠 곳은
   여기 하나다. 거꾸로 **이 저장소는 글을 쓰지 않는다** — 문체·구성·사실 검증은 저작 하네스의
   일이고, 그래서 스캐폴딩 스킬(`/new-record`)은 2026-08-02에 제거했다.
...
모든 세션에 자동 주입된다. 담는 것: 문서 지도, 절대 규칙 7가지(**글은 여기서 만들지 않는다**,
세계관 용어는 UI에만, 방랑자=손님, 본문 판타지화 금지, WORLDBOOK 우선, 슬러그 전역 유일, 영어 커밋),
...
저작 하네스(`~/projects/study-loop`)에서 쓴 학습 챕터를
블로그 연대기로 만드는 전체 흐름:

```mermaid
flowchart LR
  A["study-loop<br/>렌더된 챕터 .md"] --> B["inbox/learning/에 복사"]
  B --> C["/import-record"]
...
각 하네스가 이 계약의 저쪽 끝을 어떻게 지는지 (2026-08-03 실사고 후 정비 — TIL 두 편이
`2^10` 캐럿 표기로 발행됐다):

| 하네스 | 수식 규약이 있는 곳 | 집행 |
|---|---|---|
| study-loop | `harness/rules/registry.yaml` **R-CH-011** | 게이트 (캐럿 지수 FAIL, waivable) |
| blog-loop | `CLAUDE.md` §표기 규약 | 관례 (v0라 기계가 없다) |
| novel-loop | — | 미착수 |

| 정리/증명 | 소스는 `> 📐 **정리 N.M (이름)**: …` blockquote 마커 (learning §13·essay §4 공통) | import가 `:::theorem` 등으로 기계 변환, 📐·∎ 제거 |
| `:::` 디렉티브·✦·드롭캡 | **블로그 전용** | 소스에서 쓰지 않는다 (GitHub에서 깨짐) |
| Mermaid 코드펜스 | 표준 (GitHub도 렌더) | 무변환 |

## 5. 콘텐츠 정책 요약 (2026-07-12 확정, 2026-08-02 TIL 추가)

- **TIL (일지)**: `blog-loop`의 `til/`에서 `inbox/til/`을 거쳐 온다. 유일하게 `records`가
  아닌 **journal** 컬렉션으로 가고, `place`·`series`·`description`이 없다. 짧아도 그대로 발행.
