...
- **수식은 원고에 표준 LaTeX `$...$`/`$$...$$`로 들어와 있어야 한다.** 계약의 저쪽 끝이
  소유하는 몫이고 (`docs/harness-guide.md` §수식) 여기서는 무변환으로 통과시킨다 —
  `2^10` 같은 캐럿 표기로 들어오면 그대로 발행된다. 발견하면 고치지 말고 원 하네스에
  보고한다. (소스 규약: study-loop `format_spec` §13 · blog-loop CLAUDE.md §표기 규약)
